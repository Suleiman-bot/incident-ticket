...
function App() {
  ...
  const [form, setForm] = useState({
    ticket_id: '', category: null, sub_category: '', opened: '', reported_by: '', contact_info: '', priority: null,
    building: '', // <-- NEW FIELD
    location: '', impacted: '', description: '', detectedBy: null, detectedByOther: '', time_detected: '', root_cause: '', actions_taken: '',
    status: null, assigned_to: [], resolution_summary: '', resolution_time: '', duration: '', post_review: false,
    attachments: null, escalation_history: '', closed: '', sla_breach: false
  });
  ...
  useEffect(() => {
    if(isEditing) {
      ...
      setForm({
        ...
        reported_by: t.reported_by ?? '', contact_info: t.contact_info ?? '',
        priority: t.priority ? toOption(t.priority) : null,
        building: t.building ?? '',
        location: t.location ?? '', impacted: t.impacted ?? '', description: t.description ?? '',
        ...
        root_cause: t.root_cause ?? '', actions_taken: t.actions_taken ?? '',
        ...
        duration: t.duration ?? '', post_review: Boolean(t.post_review), attachments: null,
        escalation_history: t.escalation_history ?? '',
        closed: isoToLocalDatetime(t.closed) || (t.closed ?? ''), sla_breach: Boolean(t.sla_breach)
      });
    }
  ...
  const handleSubmit = async e => {
    e.preventDefault(); setStatusMsg(null); setSubmitting(true);
    const output = {...form};
    ...
    output.opened = dtToISO(output.opened); output.time_detected = dtToISO(output.time_detected);
    output.resolution_time = dtToISO(output.resolution_time); output.closed = dtToISO(output.closed);

    try {
      let res; const files = output.attachments; const identifier = ticketToEdit ? (ticketToEdit.id??ticketToEdit.ticket_id??ticketToEdit.id):null;
      if(files && files.length>0) {
        const formData = new FormData();
        const payload = {...output}; delete payload.attachments;
        ...
        const payload = {...output}; delete payload.attachments;
        if(isEditing){ const identifier = ticketToEdit.id??ticketToEdit.ticket_id??ticketToEdit.id; res = await api.put(`/tickets/${identifier}`,payload,{headers:{'Content-Type':'application/json'}});}
        else res = await api.post('/tickets',payload,{headers:{'Content-Type':'application/json'}});
      }

      const data = res?.data||{}; const createdId = data.ticket_id||data.id||data.ticketId||'(unknown)';
      ...
      setForm({...form, ticket_id:'', category:null, sub_category:'', opened:'', reported_by:'', contact_info:'', priority:null, building:'', location:'', impacted:'', description:'', detectedBy:null, detectedByOther:'', time_detected:'', root_cause:'', actions_taken:'', status:null, assigned_to:[], resolution_summary:'', resolution_time:'', duration:'', post_review:false, attachments:null, escalation_history:'', closed:'', sla_breach:false});
    } catch(err) {
  ...
  return (
    <Container style={{ maxWidth: 900, marginTop: 20, marginBottom: 40, fontFamily:'Arial,sans-serif'}}>
      <div style={{position:'relative'}}>
        <button type="button" onClick={toggleTheme} className="btn btn-sm btn-outline-secondary" aria-label="Toggle theme" style={{position:'absolute',right:0,top:-10,zIndex:20,transform:'translateY(-50%)'}}>{theme==='light'?'🌙 Dark':'☀️ Light'}</button>
        <div className="text-center mb-4"><img src="/KasiLogo.jpeg" alt="Company Logo" style={{maxWidth:200,height:'auto'}} /></div>
        <h2 className="text-center mb-4">Kasi Cloud Data Center Incident Ticket</h2>
        {statusMsg && <Alert variant={statusMsg.type==='success'?'success':'danger'} onClose={()=>setStatusMsg(null)} dismissible>{statusMsg.text}</Alert>}
        ...
        <Form onSubmit={handleSubmit} className="app-form">
          ...
          <Row>
            {/* <-- NEW Building Dropdown Inserted Here --> */}
            <Form.Group as={Col} md={6} className="mb-3" controlId="building">
              ...
              <Select classNamePrefix="rs"
                ...
                onChange={selected=>setForm(f=>({...f,building:selected?selected.value:''}))}
          ...
          {/* Rest of your original fields unchanged... */}
          ...
          <Form.Group className="mb-3" controlId="detectedBy">
            ...
            <Select classNamePrefix="rs" options={detectedByOptions} value={form.detectedBy} onChange={handleDetectedByChange} name="detectedBy" placeholder="-- Select --" isClearable/>
          </Form.Group>
          ...
          {/* The rest of the form continues exactly as in your original code (actions_taken, status, assigned_to, resolution_summary, resolution_time, duration, post_review, attachments, escalation_history, closed, sla_breach) */}
